...
local deathlogs = "https://discord.com/api/webhooks/977987740434051082/4LsoWxZwRVGtTk-O4ox-9jHdZl4kHFBH4_CBWZxjQ8UjVlzF8NoIiCc3ws6s7T7zJtIb"
local discord = "https://discord.gg/BFakk7bYpg"
...
RegisterCommand("nc", function(source, args)
    ...
    if args ~= nil then
        ...
        local group = User.getGroup -- Return user group (not character group)
        if group == "admin" then
            TriggerClientEvent('ncrp:noclip',source)
        else return false
    end
   end
end)
...
RegisterNetEvent("ncrp_admin:payPlayer", function(source,amount)
    local _source = source 
    local Character = VorpCore.getUser(_source).getUsedCharacter 
...
RegisterCommand('spawngang', function(source, args, rawCommand)
    if #args == 1 then 
        TriggerClientEvent('ncrp_admin:spawngang',source, args[1])
    end
end, true) -- 'true' checks if you are an admin allowed to use this command. Won't autocomplete if you're not.
