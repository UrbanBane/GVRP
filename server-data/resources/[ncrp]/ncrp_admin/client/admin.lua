...
function Admin()
    ...
    local elements = {
        ...
        { label = _U("shapeshift"), value = 'shapeshift', desc = _U("shapeshift_desc") },
    ...
    MenuData.Open('default', GetCurrentResourceName(), 'menuapi',
        ...
        function(data)
            ...
            elseif data.current.value == "shapeshift" then
...
function Shapeshift()
    MenuData.CloseAll()
    local elements = {
        { label = _U("ShapeshiftMale"), value = 'male', desc = null },
        { label = _U("ShapeshiftFemale"), value = 'female', desc = null },
        { label = _U("ShapeshiftDogs"), value = 'dogs', desc = null },
        { label = _U("ShapeshiftHorse"), value = 'horses', desc = null },
        { label = _U("ShapeshiftAnimals"), value = 'animals', desc = null },
        { label = _U("ShapeshiftBirds"), value = 'birds', desc = null },
        { label = "Use /rc to revert", value = null, desc = null },
    }
    MenuData.Open('default', GetCurrentResourceName(), 'menuapi',
        {
            title    = _U("MenuTitle"),
        ...
        function(data)
            ...
            elseif data.current.value == "female" then
                TriggerServerEvent("vorp_admin:opneStaffMenu", 'vorp.staff.Shapeshift')
                Wait(100)
                if AdminAllowed then
                    OpenFemaleMenu()
            ...
            elseif data.current.value == "dogs" then
                TriggerServerEvent("vorp_admin:opneStaffMenu", 'vorp.staff.Shapeshift')
                Wait(100)
                if AdminAllowed then
                    OpenDogsMenu()
            ...
            elseif data.current.value == "horses" then
                TriggerServerEvent("vorp_admin:opneStaffMenu", 'vorp.staff.Shapeshift')
                Wait(100)
                if AdminAllowed then
                    OpenHorsesMenu()
            ...
            elseif data.current.value == "birds" then
                TriggerServerEvent("vorp_admin:opneStaffMenu", 'vorp.staff.Shapeshift')
                Wait(100)
                if AdminAllowed then
                    OpenBirdsMenu()
...
--Handle any Blw Lawman peds spawning (hopefully)
AddEventHandler("entityCreating", function(entity)
    if GetEntityModel(entity) == "-1891246613" then
        CancelEvent()
    end
end)

function GetPlayerDistanceFromCoords(posVector, playerPed)
	local playerPos = GetEntityCoords(playerPed)
	local playerVector = vector3(playerPos.x, playerPos.y, playerPos.z)
	return #(playerVector - posVector)
end

RegisterCommand('pedid', function(source, args, rawCommand)
    print('my id:', PlayerPedId())
    print('getting all players within 100')
    for k,v in pairs(GetActivePlayers()) do
		local playerPedId = GetPlayerPed(v)
		local distance = GetPlayerDistanceFromCoords(GetEntityCoords(PlayerPedId()), playerPedId)
		if distance < 100 then
			print('player id:', playerPedId)
		end
    end
end) -- 'true' checks if you are an admin allowed to use this command. Won't autocomplete if you're not.


RegisterNetEvent('ncrp_admin:spawngang')
AddEventHandler('ncrp_admin:spawngang', function(GangName)
    local Gang = Config.GangInfo[GangName]

    local keys = keysx(Gang.pedspool)
    local players = {}
	local location = GetEntityCoords(PlayerPedId()) -- location of the executor of the /spawngang command

	print('my id:', PlayerPedId())
    for k,v in pairs(GetActivePlayers()) do
		local playerPedId = GetPlayerPed(v)
		local distance = GetPlayerDistanceFromCoords(location, playerPedId)
		if distance < 100 then
			print('player id:', playerPedId)
			SetPedRelationshipGroupHash(playerPedId, `PLAYER`)
			players[#players + 1] = playerPedId -- Collected all player ped IDs
		end
    end
    for k,v in pairs(Gang.pedinfo) do
		local targetRandomizer = math.random(1, #players)
		local targetPedId = players[targetRandomizer]

        local modelrandomizer = math.random( 1,keys)
        local modelname = Gang.pedspool[modelrandomizer]
        ...
        TaskCombatPed(createdped, targetPedId)

        --RegisterHatedTargetsAroundPed(createdped, 100)
        --TaskCombatHatedTargetsAroundPed(createdped, 100, 1, 0)

        SetPedCombatAbility(createdped, 2) -- professional
        SetPedCombatAttributes(createdped,4,true) -- "strafe based on target proximity"
        SetPedCombatAttributes(createdped,42,true) -- "can flank"
        SetPedCombatAttributes(createdped,43,true) -- "switch to advance if can't find cover"
        SetPedCombatAttributes(createdped,58,true) -- 'Disable flee from combat'
        -- SetPedCombatAttributes(createdped,27,true) -- perfect accuracy -- ouch.
        -- SetPedCombatAttributes(createdped,##, true) -- https://github.com/femga/rdr3_discoveries/tree/master/AI/COMBAT_ATTRIBUTES for numbers ##
        SetPedAccuracy(createdped, 90) -- 0 - 100
        SetPedCombatRange(createdped, 4) -- range Far from 0-near, medium, far, 4-very far
    end
...
CreateThread(function()
	TriggerEvent("chat:addSuggestion", "/spawngang", "Spawn a gang", {
		{name = "type", help = table.concat(GetGangNames(), ", ")}
	})
end)
...
function GetGangNames()
	...
	for name, _ in pairs(Config.GangInfo) do
