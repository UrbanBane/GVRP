...
elseif (Vdist(coords.x, coords.y, coords.z, 1361.23,-1304.98,77.76) < 1.0) then
    sleep = false --rhod
               TriggerEvent("enter:pd")
               if whenKeyJustPressed(keys["SPACEBAR"]) then
			   TriggerServerEvent("vorp_ml_policejob:checkjob2")
...
elseif (Vdist(coords.x, coords.y, coords.z, 2906.21,1308.71, 44.84) < 1.0) then
    sleep = false --Annesburg
                TriggerEvent("enter:pd")
                if whenKeyJustPressed(keys["SPACEBAR"]) then
                TriggerServerEvent("vorp_ml_policejob:checkjob2")
                end

elseif (Vdist(coords.x, coords.y, coords.z, 1931.76, 1947.81, 266.1) < 1.0) then
    sleep = false --Ambarino Ranger Hut
                TriggerEvent("enter:pd")
                if whenKeyJustPressed(keys["SPACEBAR"]) then
                TriggerServerEvent("vorp_ml_policejob:checkjob2")
                end	                				   
