	...
	if found then
		...
		elseif (givenItem ~= nil) and (#givenItem > 0) then
			...
			for k, v in ipairs(givenItem) do
				...
				-- Adjust value to fill up currently available inventory
				local canCarryAmount = 0
				local itemFull = false
				for collectedItem = 1, nmb do
					local canCarry = VorpInv.canCarryItems(_source, '1') -- can carry inventory space
					local canCarry2 = VorpInv.canCarryItem(_source, v, '1') -- can carry item limit
					if canCarry then
						if canCarry2 then
							VorpInv.addItem(_source, v, 1)
							canCarryAmount = canCarryAmount + 1
							Wait(10)
							canCarry2 = VorpInv.canCarryItem(_source, v, '1')
							if canCarry2 == false then -- If the last item picked up happens to fill the item capacity, notify
								itemFull = true
								break
							end
						else
			...
			for k, v in pairs(formattedGivenItems) do
				if validDisplays then
					if k > 1 then
						givenMsg = givenMsg .. Config.Language.join .. v.nmb .. " " .. givenDisplay[k] .. ((v.nmb > 1) and "s" or "")
					else
						givenMsg = givenMsg .. v.nmb .. " " .. givenDisplay[k] .. ((v.nmb > 1) and "s" or "")
					end
				end
			end
			if givenMsg ~= "" then
				TriggerClientEvent("vorp:TipRight", _source, givenMsg, 4000)
				if v ~= nil and v.itemFull == true then
					local fullMsg = "You can not carry any more"
					TriggerClientEvent("vorp:TipRight", _source, fullMsg, 4000)
				end
			end
