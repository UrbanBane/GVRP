...
local WaterTypes = {
    ...
    [28] =  {["name"] = "Arroyo De La Vibora",  ["waterhash"] = -49694339, ["watertype"] = "river"},
    -- [29] =  {["name"] = "Bahia De La Paz",      ["waterhash"] = -1168459546, ["watertype"] = "sea"},
}
            ...
            if Water == WaterTypes[k]["waterhash"]  then
                if IsPedOnFoot(PlayerPedId()) then
                    if IsEntityInWater(PlayerPedId()) then
                        ...
                        Citizen.Wait(10000)
						TriggerEvent('vorpmetabolism:changeValue', "Thirst", 250)
...
AddEventHandler('drp:rio', function()
    local _source = source
            ...
            local scene = "amb_misc@world_human_kneel_wash@male@enter"
            RequestAnimDict(scene)
            while not HasAnimDictLoaded(scene) do
                Citizen.Wait(100)
            end
            TaskPlayAnim(PlayerPedId(), scene, 'enter', 8.0, 8.0, 3000, 0, 0, true, false, false)  
            scene = "amb_misc@world_human_kneel_wash@male@idle_a"
            RequestAnimDict(scene)
            Citizen.Wait(3000)
            TaskPlayAnim(PlayerPedId(), scene, 'idle_b', 8.0, 8.0, 4000, 1, 0, true, false, false)
            scene = "amb_misc@world_human_kneel_wash@male@exit"
            RequestAnimDict(scene)
            Citizen.Wait(4000)
            TaskPlayAnim(PlayerPedId(), scene, 'exit', 8.0, 8.0, 3000, 0, 0, true, false, false)  
            Citizen.Wait(700)
