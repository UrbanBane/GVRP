...
function keys(obj)
{
    ...
    function Load() {
        ...
        for (const [x, y] of Object.entries(ordered)) {
            for (const [k, v] of Object.entries(numbers)) {
                for (const [h, j] of Object.entries(y)) {
                    
                    if (j == v ) {
                        
                        numbers[k] =  (Number(h) + 1)
                    } 
                }
            }
        }
        ...
        $('input').change(function() {
            $.post('http://syn_clothing/updateClothes', GetOutput());
        });
    ...
    $('.yes').on('click', function(e) {
        e.preventDefault();
        $.post('http://syn_clothing/saveClothes', GetOutput());
    });

    $('.exit').on('click', function(e) {
        e.preventDefault();
        $.post('http://syn_clothing/exit');
    });

    // Rotate player
    $(document).keypress(function(e) {
        if (e.which == 113) { // Q pressed
            $.post('http://syn_clothing/heading', JSON.stringify({
                value: -10
            }));
        }
        if (e.which == 120) { // x pressed
            $.post('http://syn_clothing/defcam');
        }
		...
		if (e.which == 119) { // w pressed
            $.post('http://syn_clothing/camera', JSON.stringify({
                offset: 0.05
            }));
        }
