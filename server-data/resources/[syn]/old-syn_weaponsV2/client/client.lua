...
function wepsmen() 
	...
	for k,v in pairs(playerweps) do 
		table.insert( elements, {label = v.name, value = v, desc = Config2.Language.presstobuy})
	end
   MenuData.Open('default', GetCurrentResourceName(), 'menuapi',
	{
...
AddEventHandler("syn_weapons:givecomp", function(components,id,hash)
	local ped = PlayerPedId()
	TriggerEvent("syn_weapons:wepcomp",2)
	for x,y in pairs(WeaponComponentz) do 
		for k,v in pairs(components) do 
			if x == k then 
				for g,m in pairs(y) do
					RemoveWeaponComponentFromPed(PlayerPedId(), GetHashKey(m.item), hash)
				end
			end
		end
	end
	...
	GiveWeaponComponentToEntity(PlayerPedId(), GetHashKey(components["WRAP_TINT"]), hash, true)
end) 
