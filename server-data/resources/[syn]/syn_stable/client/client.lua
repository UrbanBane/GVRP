...
Citizen.CreateThread(function()
    if Config.disablehorsecallinjail then 
        while true do
            Citizen.Wait(500)
            if playerjob ~= nil then 
	    	    ...
	    	    local ZoneTypeId = 10
  	    	    ...
  	    	    if current_district == 1308232528 then
                    if Config.allowpolicecall  and conx(Config.policejobs,playerjob) then 
                        disablehorse = false 
                    else
                        disablehorse = true 
                    end
	    	    elseif disablehorse then 
	    	    	disablehorse = false 
  	    	    end
            end
        end
...
RegisterNUICallback(
    ...
    function(data)
        ...
        --print(json.encode(data.HorseComp))
        ...
        if componentsHorse ~= '[]' then 
            for _, Key in pairs(componentsHorse) do
                ...
                if contains(HorseComp, key) == 'Saddles' then 
                    SaddlesUsing = Key
                elseif contains(HorseComp, key) == 'Saddlecloths' then
                    SaddleclothsUsing = Key
                elseif contains(HorseComp, key) == 'Stirrups' then
                    StirrupsUsing = Key
                elseif contains(HorseComp, key) == 'Bags' then
                    BagsUsing = Key
                elseif contains(HorseComp, key) == 'Manes' then
                    ManesUsing = Key
                elseif contains(HorseComp, key) == 'HorseTails' then
                    HorseTailsUsing = Key
                elseif contains(HorseComp, key) == 'AcsHorn' then
                    AcsHornUsing = Key
                elseif contains(HorseComp, key) == 'AcsLuggage' then
                    AcsLuggageUsing = Key
                elseif contains(HorseComp, key) == 'Lantern' then 
                    LanternUsing = Key
                elseif contains(HorseComp, key) == 'Mask' then
                    MaskUsing = Key
                elseif contains(HorseComp, key) == 'Mustache' then
                    MustacheUsing = Key
                elseif contains(HorseComp, key) == 'Horsebridles' then
                    HorsebridlesUsing = Key
                elseif contains(HorseComp, key) == 'Horseholster' then
                    HorseholsterUsing = Key
                elseif contains(HorseComp, key) == 'Horseshoes' then
                    HorseshoesUsing = Key
                end
...
RegisterCommand("nuioff", function(source, args, rawCommand)
    print("attempting to fix nui")
    SetNuiFocus(false, false)
    PlayCloseSound();
end, false)

RegisterCommand("closestable", function(source, args, rawCommand)
    SetNuiFocus(false, false)
    SendNUIMessage(
        {
            action = "hide"
...
function Initiatewagon(atCoords)
    print('initializing2: '..tostring(initializing2))
