...
local lastFurnitureMenu = {}
local lastFurnitureCategory = {}
        ...
        if placefurniture then
            ...
            if dist > rangex then
                ...
                if whenKeyJustPressed(Config.keys["4"]) then
                    ...
                    buyfurnimenu3(lastFurnitureMenu, lastFurnitureCategory)
                end
            else
                ...
                if whenKeyJustPressed(Config.keys["ENTER"]) then
                    if placedobject then
                        ...
                        DeleteObject(objectxyz)
                        Citizen.Wait(500)
                        confirmmenu("confirmfurniturebuy","buyfurnimenu3")
                    end
                end
            end
        end  
	...
	function(data, menu)
        ...
        if data.current.value == "yes" then
            ...
            elseif type == "buyhome" then 
                ...
                if Config.maxhouses > numberofownedhouses or (playerjob == Config.synsocietyjob and tonumber(playerrank) >= tonumber(Config.synsocietyjobrank)) or playergroup == "admin" or playergroup == "superadmin" then 
        ...
        if data.current.value == "no" then
            if type == "confirmfurniturebuy" then
                TriggerServerEvent("syn_housing:closemenu",globalhouseid)            
                inmenu = false
                MenuData.CloseAll()
            else
...
function sellfurnimenu() 
	...
	function(data, menu)
        if data.current == "backup" then 
            ...
            local furniid = data.current.value.id
            local label = data.current.value.name
            local sellprice = data.current.value.price * Config.furnituresellrate
            payPlayer(sellprice)
...
function payPlayer(amount)
    ...
    TriggerServerEvent("ncrp_admin:payPlayer", player, amount)
end

function keysort(tableToSort)
    local sortedKeys = {}
...
function buyfurnimenu() 
    ...
    categories = keysort(Config.furniture)
    for key, category in ipairs(categories) do
        table.insert( elements, {label = category, value = Config.furniture[category] , desc = ""})
    end
    MenuData.Open('default', GetCurrentResourceName(), 'menuapi',
	{
		title    = Config.Language.managehouse,
...
function buyfurnimenu2(furniSubGroup)
    MenuData.CloseAll()
    local elements = {}
    local subgroups = {}
    
    subgroups = keysort(furniSubGroup)
    for key, subgroup in ipairs(subgroups) do
        table.insert( elements, {label = subgroup, value = furniSubGroup[subgroup] , desc = ""})
    end
    MenuData.Open('default', GetCurrentResourceName(), 'menuapi',
	{
		title    = Config.Language.managehouse,
	...
	function(data, menu)
        if data.current == "backup" then 
            ...
            buyfurnimenu3(data.current.value, furniSubGroup) 
        end
    end,
	function(data, menu)
		menu.close()
...
function buyfurnimenu3(furnigroup, furniSubGroup) 
    lastFurnitureMenu = furnigroup
    lastFurnitureCategory = furniSubGroup
    MenuData.CloseAll()
    local elements = {}
    local furniture = {}
    furniture = keysort(furnigroup)

    for key, furniture in pairs(furniture) do
        local furnitureElement = furnigroup[furniture]
        table.insert( elements, {name = furniture, label = furniture..Config.Language.cost2..furnitureElement.cost.." ", value = furnitureElement , desc = ""})
    end
    MenuData.Open('default', GetCurrentResourceName(), 'menuapi',
	{
		title    = Config.Language.managehouse,
		...
		align    = 'top-left',
		elements = elements,
        lastmenu = "buyfurnimenu2",
	},
	function(data, menu)
        if data.current == "backup" then 
            _G[data.trigger](furniSubGroup)
        else
            furnitem = data.current.value.hash
            furnitemcost = data.current.value.cost
            furniname = data.current.name
            placefurniture = true
            MenuData.CloseAll()
            FreezeEntityPosition(PlayerPedId(),false)
            ClearPedTasks(PlayerPedId())
