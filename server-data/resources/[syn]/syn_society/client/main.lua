...
Citizen.CreateThread(function()
  ...
  while true do
        ...
        if closestPlayer ~= -1 and closestDistance <= 2.0 then
          ...
          for k, v in pairs(Config.jobs) do
            if k == playerjob then
              ...
              if Config.vorp then
                TriggerEvent("syn_Container:openinventory", name, id) --- trigger to open any created inventory
              elseif Config.redem then
    ...
    elseif WarMenu.IsMenuOpened('hire') then
      WarMenu.CloseMenu()
      if Config.vorp then
        TriggerEvent("vorpinputs:getInput", Config.Language.confirm, Config.Language.playerid, function(cb)
    ...
    elseif WarMenu.IsMenuOpened('setrank') then
      for i = 1, #online do
        if WarMenu.Button("ID: " .. online[i].no .. "//  " .. online[i].name .. Config.Language.listrank .. online[i].jobgrade) then
          if Config.vorp then
            TriggerEvent("vorpinputs:getInput", Config.Language.confirm, Config.Language.rank, function(cb)
        ...
        if k == playerjob then
          for i = 0, v.bossrank do
            if WarMenu.Button("" .. i) then
              WarMenu.CloseMenu()
              if Config.vorp then
                TriggerEvent("vorpinputs:getInput", Config.Language.confirm, Config.Language.salarys, function(cb)
    ...
    elseif WarMenu.IsMenuOpened('ledger') then
      ...
      if WarMenu.Button(Config.Language.depositcash) then
        ...
        if closestPlayer ~= -1 and closestDistance <= 2.0 then
          ...
          if Config.vorp then
            TriggerEvent("vorpinputs:getInput", Config.Language.confirm, Config.Language.depositcash, function(cb)
      ...
      if WarMenu.Button(Config.Language.withdrawcash) then
        ...
        if closestPlayer ~= -1 and closestDistance <= 2.0 then
          ...
          if Config.vorp then
            TriggerEvent("vorpinputs:getInput", Config.Language.confirm, Config.Language.withdrawcash, function(cb)
...
AddEventHandler("syn_Container:openinventory", function(name, id)
    ...
    for k,v in pairs(Config.alerts) do 
        RegisterCommand(v.command, function(source, args, rawCommand)
            if not cooldown then 
                ...
                TriggerEvent("vorp:TipBottom", Config.Language.alertsent, 2000)
                cooldown = true
            else
              TriggerEvent("vorp:TipBottom", Config.Language.alertcooldown, 2000)
            end
        end, false)
    end
  end
...
AddEventHandler("syn_alert:findclosedoctor", function(data,alert,blip,doctor)
    ...
    for k,v in pairs(data) do 
        local dist = GetDistanceBetweenCoords(v.coords.x,v.coords.y,v.coords.z, coordsx.x,coordsx.y,coordsx.z, true)
