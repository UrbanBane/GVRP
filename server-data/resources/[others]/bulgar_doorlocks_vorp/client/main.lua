...
RegisterNetEvent('bulgar_doorlocks_vorp:lockpick')
AddEventHandler('bulgar_doorlocks_vorp:lockpick', function(source, doorID)	
    Wait(100)
	if Config.DoorList[doorID].locked == true then
		openui()
		...
		if pass == true then
			closeui()
			pass = nil 
            TriggerServerEvent("bulgar_doorlocks_vorp:lockpickUpdateState", doorID, false, function(cb) end)
		elseif pass == false then 
			closeui()
			pass = nil 
			TriggerServerEvent("bulgar_doorlocks_vorp:brokenlockpick", source, function(cb) end)
		end
	end
end)
