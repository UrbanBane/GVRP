...
RegisterCommand("cloak", function(source, args, rawCommand)
    ExecuteCommand("cloak")
    ExecuteCommand("clothingmenu")
end)
...
RegisterCommand('salute', function(source, args, rawCommand)
    local  scene = "ai_gestures@gen_male@standing@silent"
    RequestAnimDict(scene)
    while not HasAnimDictLoaded(scene) do
        Citizen.Wait(100)
    end
    TaskPlayAnim(PlayerPedId(), scene, 'silent_casual_salute_r_002', 1.0, -1.0, 2250, 31, 0, true, 0, false, 0, false)
end, false)
