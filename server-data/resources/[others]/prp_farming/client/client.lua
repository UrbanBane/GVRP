		...
		if myPlants[1] ~= nil then
			...
			for k, v in ipairs(myPlants) do
				if GetDistanceBetweenCoords(v.x, v.y, v.z, pos.x, pos.y, pos.z, true) < 25.0 then
					...
					if v.stage == 3 then
						DrawText3D(v.x, v.y, v.z, 'Press [~e~G~q~] to Harvest!')
					end
					if v.prompt then
						if Citizen.InvokeNative(0x91AEF906BCA88877, 0, 0x760A9C6F) then
							local key = k
							harvestPlant(key)
...
function animacion()
    ...
    if isMale then
        ...
        local righthand = GetEntityBoneIndexByName(ped, "SKEL_R_finger42")
        TriggerEvent("vorp:TipRight", "Raking the Soil...", 3000)
        Anim(ped, "amb_work@world_human_farmer_rake@male_a@stand_enter_withprop", "enter_back_lf", 4000, 0)
        Wait(2000)
        local rake = CreateObject(GetHashKey('p_rake02x'), x, y, z, true, true, true)
        AttachEntityToEntity(rake, ped, righthand, 0.08, 0.5--[[up/down]], -0.92--[[along handle]], -14.0--[[pitch]], 25.0 --[[yaw]], -128.0 --[[roll]], true, true, false, false, 0, true)
        Wait(2000)
        Anim(ped, "amb_work@world_human_farmer_rake@male_a@idle_a", "idle_b", 6000, 1)
        Wait(6000)
        Anim(ped, "amb_work@world_human_farmer_rake@male_a@stand_exit_withprop", "exit_front", 3000, 0)
        Wait(1000)
        DeleteEntity(rake)

        TriggerEvent("vorp:TipRight", "Planting Seeds...", 3000)
        Anim(ped, "script_story@gua3@unapproved", "gardener_plant@male@enter", 5000, 2)
        Wait(5000)
        Anim(ped, "script_story@gua3@unapproved", "gardener_plant@male@exit", 5000, 0)
