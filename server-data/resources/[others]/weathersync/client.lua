...
function IsInSouthernRegion(x, y, z)
	return (x >= 2000 and y <= 0) or ( x >= 2000 and y <= -165) or (x >= 1675 and y <= -180) or ( x >= 1080 and y <= -350) or (x >= 566 and y <= -530) or (x >= 180 and y <= -600)
end
...
function TranslateWeatherForRegion(weather)
	...
	elseif weather == 'snow' then
		...
		if IsInSouthernRegion(x, y, z) then 
			return 'rain'
		end
	...
	elseif weather == 'blizzard' or weather == 'groundblizzard' then
		if IsInGuarma(x, y, z) then
			return 'sunny'
		end
		if IsInSouthernRegion(x, y, z) then 
