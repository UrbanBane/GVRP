local healing = true --[ confusing variable - was originally set to false so opp should turn it off ]--
        ...
        for i = 1, #locations do
            ...
            if IsPlayerNearCoords(locations[i].x, locations[i].y, locations[i].z, 1) then
                ...
                if not healing then
                    ...
                    DisplayNPCText("Hi! How can I help you?", 0.35, 0.35, 0.5, 0.88, 255, 255, 255, 0)
                    TriggerEvent("vorp:TipBottom", "Press [C] to be treated by your doctor.", 2000)
                    if IsControlJustReleased(0, 0x9959A6F0) then
                        if IsEntityDead(PlayerPedId()) then
		    				...
		    				TriggerEvent("vorp:NotifyLeft", "~t6~You've been treated "," It cost 40$.", "generic_textures", "tick", 8000)
                        else
                            HealPlayer()
                            TriggerServerEvent('ambulancejob:CobrarRevivir')
		    				TriggerEvent("vorp:NotifyLeft", "~t6~You've been treated "," It cost 40$.", "generic_textures", "tick", 8000)
...
function HealPlayer()
    ...
    TriggerEvent("vorp:NotifyLeft", "~t6~You've been treated","Be safe out there now.", "generic_textures", "tick", 8000)
end
