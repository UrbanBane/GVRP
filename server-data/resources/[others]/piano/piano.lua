...
local time_between_tip = 75000  -- this is the number of miliseconds between tips and xp default is 30 seconds // Set to 75 seconds currently
...
---------------------------Valentine ----------------------------------------
        if IsPlayerNearCoords(-312.3, 799.01, 118.46) then
            if not play then 
            DrawTxt("[~e~SPACEBAR~q~] to play piano for tips, [~e~W~q~] to stop", 0.50, 0.95, 0.7, 0.5, true, 223, 44, 53, 255, true)
            end
			...
			elseif rnd <=75 then
                        	TaskStartScenarioAtPosition(PlayerPedId(), GetHashKey('PROP_HUMAN_PIANO_RIVERBOAT'), -312.22 - 0.08, 799.01, 118.43 + 0.03, 102.37, denduration, true, true, 0, true)
			else
        ...
        if IsPlayerNearCoords(2632.282, -1232.0, 54.19) then
            ...
            if IsControlJustPressed(0, 0xD9D0E1C0) then
		        ...
		        TaskStartScenarioAtPosition(PlayerPedId(), GetHashKey('PROP_HUMAN_ABIGAIL_PIANO'), 2631.946 - 0.22, -1232.617 + 0.3, 54.19 - 0.46, 67.788, duration, true, true, 0, true)
                	...
                	TaskStartScenarioAtPosition(PlayerPedId(), GetHashKey('PROP_HUMAN_PIANO_UPPERCLASS'), 2631.946 - 0.22, -1232.617 + 0.3, 54.19 - 0.46, 67.788, duration, true, true, 0, true)
		end
		while play == true do
			Citizen.Wait( time_between_tip )      
			TriggerServerEvent('piano:AddSomeMoney')
...
--------------------------- ST Denis  doyels ----------------------------------------            
		...
		if ismale == false then
		        TaskStartScenarioAtPosition(PlayerPedId(), GetHashKey('PROP_HUMAN_ABIGAIL_PIANO'), 2799.582 + 1.0, -1163.929 - 0.3, 47.92 - 0.40, 237.40, duration, true, true, 0, true)
              		...
              		TaskStartScenarioAtPosition(PlayerPedId(), GetHashKey('PROP_HUMAN_PIANO_SKETCHY'), 2799.582 + 1.0, -1163.929 - 0.3, 47.92 - 0.40, 237.40, duration, true, true, 0, true)
		end
		while play == true do
			Citizen.Wait( time_between_tip )      
			TriggerServerEvent('piano:AddSomeMoney')
