...
local LawTrackingEnabled = false
    ...
    while true do
        ...
        if IsPlayerTargettingAnything(id) then
			...
			if PromptHasStandardModeCompleted(AttackPrompt[entity]) then
					...
					repeat
						Wait(4000)
						retval = IsEntityDead(entity)
					until(retval == true)
					Wait(1000)
					ClearPedTasks(currentPetPed)
					SetRelationshipBetweenGroups(3, GetPedRelationshipGroupHash(currentPetPed))
					followOwner(currentPetPed,ped,false)
			end
			if PromptHasStandardModeCompleted(TrackPrompt[entity]) then
				if LawTrackingEnabled then
					TrackTarget(entity)
					print("canLawTrack - entity: "..entity)
				else				
					if GetPedType(entity) == 28 and not IsPedAPlayer(entity) then
						TrackTarget(entity)
					else
						TriggerEvent( 'UI:DrawNotification', "Your pet cannot track this target")
					end
					print("canTrack - entity: "..entity)
				end
				
				print(TrackingEnabled)
				print(LawTrackingEnabled)
			end		
...
RegisterCommand("fleehuntpet", function(source, args, rawCommand) --  COMMAND
    local _source = source
    local ped = PlayerPedId()
	TriggerEvent('rdn_companions:putaway')
end)

RegisterCommand("callhuntpet", function(source, args, rawCommand) --  COMMAND
    local _source = source
    local ped = PlayerPedId()
	TriggerServerEvent('rdn_companions:loaddog')
end)
...
AddEventHandler('rdn_companions:spawndog', function (dog,skin,isInShop,xp,canTrack,canLawTrack)
	if dogActive == false then
		if recentlySpawned <= 0 then
		...
		LawTrackingEnabled = canLawTrack
		...
		ShowNotification("Your dog is already out")
		return
	end
