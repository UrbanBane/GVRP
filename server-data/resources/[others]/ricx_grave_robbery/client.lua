...
AddEventHandler("ricx_grave_robbery:dig", function(id)
    ...
    if digging then
        EndShovel()
    elseif hour >= 4 and hour < 22 then
        TriggerEvent("Notification:left_grave_robbery", TEXTS.GraveRobbery, TEXTS.DayTime, TEXTURES.alert[1],
        TEXTURES.alert[2], 2000)
    else
...
AddEventHandler("ricx_grave_robbery:start_dig", function(id)
    ...
    Citizen.Wait(1000)
    for i,v in pairs(Config.JobsToAlert) do
        jobs = {v}
        blip = {
            blipsprite = 2119977580,
            Name = 'Grave Robbery',
            coords = GetEntityCoords(pedp)
        }
        if Town == nil then Town = 'by' end
		TriggerServerEvent("syn_alert:sendalert",jobs,"I saw someone robbing a grave near " .. Town,blip,false)
    end
end)
